...
class Orchestrator {
  ...
  connect(url, options = {}) {
    ...
    let socket = socket_io_client(url)
                  ...
                  .on('/create', (msg = {}, cb) => {
                    let { remote, sha, token, bearer, single_use }  = msg
                      , { key, hash }                               = make_sdk.key(remote, sha, { single_use })
                      ;

                    // let { blob, filename, cache, tailf } = msg;
                    ...
                    return codedb_sdk
                            ...
                            .then((result) => {
                              ...
                              return Sandbox
                                      .create(name, image, { replicas, secret, limits : { memorybytes, nanocpus }, make_hash : hash })
                                      .then((result) => {
                                        let { ID : id } = result;
